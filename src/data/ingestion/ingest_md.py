...
def detect_content_type(content: str, file_path: str) -> str:
    ...
    if 'content_type: character' in content_lower or 'content_category: character' in content_lower:
        return 'operator'
    if 'content_type: operator' in content_lower:
        return 'operator'
...
def enrich_metadata(doc, original_path: str, is_distilled: bool = False):
    ...
    enriched_metadata = {
        ...
        "doc_type": doc_type,
        "is_distilled": "true" if is_distilled else "false",
...
def ingest_markdown(docs_dir: str, collection_name: str, host: str, port: int,
                    ...
                    use_smart_chunking: bool = True, is_distilled: bool | None = None) -> int:
    """
    ...
    Args:
        ...
        is_distilled: Whether this is distilled content. If None, auto-detect from path.

    Returns:
        Number of chunks ingested
    """
    # Auto-detect is_distilled from path if not explicitly set
    if is_distilled is None:
        is_distilled = 'distilled' in docs_dir.lower() or 'prts_distilled' in docs_dir.lower()
    
    print(f"[Info] is_distilled mode: {is_distilled}")
    ...
    # Overwrite chunk size/overlap for distilled content if requested
    if is_distilled:
        from src.config.optimized_retrieval_config import get_chunk_config, BALANCED_CONFIG
        distilled_config = get_chunk_config("distilled", BALANCED_CONFIG)
        chunk_size = distilled_config["chunk_size"]
        chunk_overlap = distilled_config["chunk_overlap"]
        print(f"[Info] Overriding chunk settings for distilled content: size={chunk_size}, overlap={chunk_overlap}")
    ...
    # Store is_distilled in a variable for use in enrichment
    _is_distilled = is_distilled
    for chunk in chunks:
        # 获取原始文件路径
        original_path = chunk.metadata.get('source', 'unknown')

        # 丰富metadata，传入 is_distilled 标记
        enriched_chunk = enrich_metadata(chunk, original_path, is_distilled=_is_distilled)
    ...
    for i in range(0, total_chunks, batch_size):
        ...
        # Generate unique IDs based on file path + chunk index to prevent duplicates
        # ChromaDB will overwrite documents with the same ID instead of creating duplicates
        ids = []
        for idx, chunk in enumerate(batch):
            source = chunk.metadata.get('source', 'unknown')
            # Use MD5 hash of (source_path + global_chunk_index) as unique ID
            chunk_id = hashlib.md5(f"{source}_chunk_{i + idx}".encode()).hexdigest()
            ids.append(chunk_id)
        ...
        if i == 0:
            ...
            _vs = Chroma.from_documents(
                ...
                client=client,
                ids=ids,  # Add unique IDs to prevent duplicates
                collection_metadata=collection_metadata,
            )
        else:
            # 后续批次：添加到现有collection
            _vs.add_documents(documents=batch, ids=ids)  # Add IDs here too
...
def parse_args() -> argparse.Namespace:
    ...
    # Default to a local 'docs/prts_distilled' directory
    base_dir = os.path.dirname(os.path.abspath(__file__))
    default_docs_dir = os.path.join(base_dir, "docs", "prts_distilled")
    parser.add_argument("--docs-dir", type=str, default=default_docs_dir, help="Directory to recursively read *.md files")
    parser.add_argument("--collection", type=str, default=os.getenv("CHROMA_COLLECTION", "prts_wiki"), help="Chroma collection name")
    ...
    parser.add_argument("--is-distilled", action="store_true", help="Mark documents as distilled content (for unified collection)")
...
def main() -> None:
    ...
    try:
        ...
        # Determine is_distilled: explicit flag takes precedence, otherwise None (auto-detect)
        is_distilled = True if args.is_distilled else None
