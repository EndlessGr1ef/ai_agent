...
请解释上述选中内容。`;
    ...
    if (!chrome.runtime?.id) {
      setLoading(false);
      setError('扩展已更新，请刷新页面');
      return;
    }

    try {
      chrome.runtime.sendMessage(
        { action: 'queryAI', payload: { selection, context, pageUrl, pageTitle } },
        (response) => {
          setLoading(false);
          ...
          <div style={contentStyle}>
            ...
            {!loading && result && (
              <div style={footerStyle}>
                ...
                <button style={actionButtonStyle} title="朗读">🔊</button>
                <button style={actionButtonStyle} title="点赞">👍</button>
                <button style={actionButtonStyle} title="踩">👎</button>
              </div>
            )}
          </div>
        </div>
